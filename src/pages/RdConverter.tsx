      ...
      <div className="calc-grid">
        <div className="calc-inputs">
          ...
          <div className="input-section">
            <label>Current Year R&D Expense</label>
            <input type="number" value={inputs.currentYearRd} onChange={e => update('currentYearRd', parseFloat(e.target.value) || 0)} />
            <label>Amortizable Life (years)</label>
            <input type="number" value={inputs.amortizableLife} onChange={e => update('amortizableLife', parseInt(e.target.value) || 1)} />
          </div>
          <h2>Historical R&D</h2>
          <div className="input-section">
            {inputs.historicalRd.map((h, i) => (
              <div key={i}>
                <label>{h.yearsAgo} year(s) ago</label>
                <input type="number" value={h.rdExpense} onChange={e => updateHistorical(i, parseFloat(e.target.value) || 0)} />
              </div>
            ))}
          </div>
          <button onClick={() => setResult(computeRdConversion(inputs))} className="btn-primary calc-btn">Convert R&D</button>
        </div>
        {result && (
          <div className="calc-results">
            <h2>R&D Conversion Result</h2>
            <div className="result-summary">
              <div className="result-card"><span className="result-label">Unamortized R&D Asset</span><span className="result-number">{fmtCompact(result.totalUnamortizedRd)}</span></div>
              <div className="result-card"><span className="result-label">Annual Amortization</span><span className="result-number">{fmtCompact(result.currentYearAmortization)}</span></div>
              ...
              <div className="result-card"><span className="result-label">Adjustment to Equity</span><span className="result-number">{fmtCompact(result.adjustmentToEquity)}</span></div>
            </div>
          </div>
        )}
      </div>
